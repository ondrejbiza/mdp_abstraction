...
def partition_improvement(partition, classifier, distance, k):
    ...
    for state_block_idx in range(len(state_partition)):
        ...
        while flag:
            ...
            for new_block in new_partition:
                ...
                tmp_new_partition = split(new_block, state_block_idx, new_partition, classifier)
...
def full_partition_iteration(gather_experience, classifier, distance, k, num_steps, reset=False, reclassify=False,
                             visualize_state_action_partition=None, visualize_state_partition=None):

    assert not (reset and reclassify)
    ...
    for _ in range(num_steps):
        ...
        # add experience
        if reset or len(state_action_partition) == 0:
