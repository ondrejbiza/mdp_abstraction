...
def split(block, states, partition, mdp, f):
    """
    ...
    for state, action in block:
        ...
        state_block = mdp.get_state_block(state)
        state_action_block = mdp.get_state_action_block(state, action)

        reward = mdp.R[state_block]
        next_state = mdp.P[state_action_block]
...
def partition_iteration(mdp, g, f, d, k):
    """
    ...
    exp = g(mdp, 100)
    state_actions = set()
    for transition in exp:
        state_actions.add((transition[0], transition[1]))

    partition = {frozenset(state_actions)}
